...
func TestNewModulesFromConfig(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			resultFile: testFolder + "expected.module",
			actualFile: testFolder + "actual.module",
			fileName:   testFolder + "20x10_print_ws2801.json",
		},
	}
	for _, c := range cases {
...
func TestConfigRotate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			height:   2,
			rotation: Rotate90,
			input:    image.Point{X: 1, Y: 0},
			expected: image.Point{X: 1, Y: 1},
		},
		{
			desc:     "2x3_0Grad_0_0",
			width:    2,
		...
		},
		{
			...
			rotation: Rotate180,
			input:    image.Point{X: 0, Y: 0},
			expected: image.Point{X: 1, Y: 2},
		},
		{
			desc:     "2x3_270Grad_0_0_with_offset",
			width:    2,
			height:   3,
			origin:   image.Point{X: 10, Y: 3},
			rotation: Rotate270,
			input:    image.Point{X: 0, Y: 0},
			expected: image.Point{X: 0, Y: 1},
		},
		{
			desc:     "2x3_90Grad_0_0",
			width:    2,
		...
		},
		{
			...
			height:   10,
			rotation: Rotate90,
			input:    image.Point{X: 9, Y: 0},
			expected: image.Point{X: 9, Y: 9},
		},
		{
			...
			height:   9,
			rotation: Rotate90,
			input:    image.Point{X: 8, Y: 1},
			expected: image.Point{X: 7, Y: 8},
		},
	}
	for _, c := range cases {
		t.Run(c.desc, func(t *testing.T) {
